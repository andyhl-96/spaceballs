...
def run_viz(pfunc: str, N: int, ball_size: float, ball_mass: float):
    dt = 0.001
    global vis
    ...
    while True:
        ...
        dynam = dynamics(Body.dynamics_matrix, dt, pfunc, np.zeros((len(Body.dynamics_matrix), 3)), 0.5)
...
def main():
    ...
    root.mainloop()
    run_viz(app.variables["pfunc"], 
            app.variables["N"], 
            app.variables["ball_size"], 
            app.variables["ball_mass"])
